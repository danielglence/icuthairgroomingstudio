...
const gallery = [
  ...
  { src: "/manus-storage/i-cut-interior_096673db.jpg", alt: "Dark modern grooming studio interior with barber chair and brass mirror", label: "The studio" },
  { src: "/manus-storage/i-cut-styling_6702c40d.jpg", alt: "Barber shaping a client's haircut with comb and shears", label: "The craft" },
  { src: "/manus-storage/i-cut-tools_ff3bbc55.jpg", alt: "Premium barber scissors, comb and straight razor arranged on a work surface", label: "The tools" },
  { src: "/manus-storage/i-cut-beard_04ad350c.jpg", alt: "Precisely shaped beard being finished with a soft grooming brush", label: "The finish" },
  { src: "https://images.unsplash.com/photo-1521590832167-7bcbfaa6381f?auto=format&fit=crop&w=1200&q=85", alt: "Brightly lit contemporary salon interior with styling stations", label: "The atmosphere" },
];
...
const services = [
  ...
  { title: "Hair Styling", description: "Modern styling with the finish, movement, and hold your look calls for.", icon: Sparkles },
  { title: "Beard Trim & Styling", description: "Clean lines, balanced proportions, and a finish that feels unmistakably yours.", icon: Scissors },
  { title: "Hair & Beard Combo", description: "A complete grooming reset, tuned as one cohesive signature look.", icon: Check },
  { title: "Hair Wash & Finish", description: "A fresh wash followed by a polished finish, ready for wherever the day leads.", icon: Sparkles },
  { title: "Kids’ Haircut", description: "Patient, comfortable grooming for younger clients and their own sense of style.", icon: Scissors },
];
...
export default function Home() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (validate(event.currentTarget)) setSubmitted(true);
  };
    ...
    <div className="min-h-screen overflow-x-hidden bg-ink text-ivory selection:bg-gold selection:text-ink">
      <header className={`fixed inset-x-0 top-0 z-40 transition-all duration-300 ${scrolled ? "border-b border-white/10 bg-ink/95 backdrop-blur-xl" : "bg-transparent"}`}>
        <div className="container flex h-[76px] items-center justify-between">
          <button onClick={() => goTo("home")} className="group flex items-center gap-3" aria-label="I Cut Hair Grooming Studio home">
            <img src="/manus-storage/i-cut-mark_899c2e1e.png" alt="I Cut monogram" className="h-9 w-9 object-contain" />
            <span className="font-sans text-[13px] font-bold tracking-[0.3em] text-ivory">I CUT</span>
          </button>
          <nav className="hidden items-center gap-7 lg:flex" aria-label="Primary navigation">
            {navItems.map((item) => <button key={item} onClick={() => goTo(item.toLowerCase())} className="nav-link">{item}</button>)}
          </nav>
        ...
        <AnimatePresence>
          {menuOpen && <motion.nav initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="border-t border-white/10 bg-ink px-5 py-5 lg:hidden" aria-label="Mobile navigation">
            {navItems.map((item) => <button key={item} onClick={() => goTo(item.toLowerCase())} className="block w-full border-b border-white/10 py-3 text-left font-sans text-xs uppercase tracking-[0.18em] text-ivory/75 last:border-0">{item}</button>)}
      ...
      <main>
        <section id="home" className="hero-section relative flex min-h-[720px] items-end overflow-hidden pb-20 pt-32 sm:min-h-screen lg:pb-28">
          ...
          <div className="container relative z-10 grid items-end gap-12 lg:grid-cols-[1fr_280px]">
            <div className="max-w-3xl">
              ...
              <motion.p initial={reduce ? false : { opacity: 0, y: 18 }} animate={reduce ? undefined : { opacity: 1, y: 0 }} transition={{ duration: .7, delay: .36, ease }} className="mt-7 max-w-lg text-base leading-7 text-ivory/70 sm:text-lg">Expert haircuts and modern grooming in a studio built around your individual style.</motion.p>
              <motion.div initial={reduce ? false : { opacity: 0, y: 18 }} animate={reduce ? undefined : { opacity: 1, y: 0 }} transition={{ duration: .7, delay: .48, ease }} className="mt-9 flex flex-wrap gap-3"><button onClick={() => goTo("appointment")} className="gold-button">Reserve your chair <ArrowRight size={15} /></button><a href={BUSINESS.maps} target="_blank" rel="noreferrer" className="outline-button">Get directions <MoveUpRight size={15} /></a></motion.div>
            </div>
        ...
        <section id="about" className="section-shell bg-ink">
          <div className="container grid gap-14 lg:grid-cols-[.8fr_1.2fr] lg:gap-24">
            ...
            <Reveal delay={.1} className="lg:pt-16"><p className="max-w-xl text-xl leading-9 text-ivory/80">A modern local grooming destination for people who care about the details. Every cut is considered, every finish is personal, and every visit is designed to leave you feeling more like yourself.</p><p className="mt-7 max-w-xl leading-7 text-muted">From the first consultation to the final mirror check, I Cut brings precision, comfort, and an unhurried studio experience to Mudavoor.</p><div className="mt-10 h-px w-28 bg-gold" /></Reveal>
          </div>
          <div className="container mt-20 grid gap-px bg-white/10 sm:grid-cols-3">{[{ icon: Scissors, title: "Expert grooming", text: "Clean technique and an eye for proportion." }, { icon: Sparkles, title: "Personalised style", text: "A look shaped around your features and routine." }, { icon: Clock3, title: "Relaxed experience", text: "A calm chair, thoughtful finish, no rush." }].map((feature, i) => { const Icon = feature.icon; return <Reveal key={feature.title} delay={i * .08} className="bg-ink p-8 lg:p-10"><Icon className="text-gold" size={22} strokeWidth={1.5} /><h3 className="mt-8 font-serif text-2xl text-ivory">{feature.title}</h3><p className="mt-3 text-sm leading-6 text-muted">{feature.text}</p></Reveal>; })}</div>
        </section>
